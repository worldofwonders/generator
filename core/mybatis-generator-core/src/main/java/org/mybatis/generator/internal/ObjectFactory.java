...
import org.mybatis.generator.internal.types.ExtendedJavaTypeResolverDefaultImpl;
...
public class ObjectFactory {
    ...
    public static JavaTypeResolver createJavaTypeResolver(Context context,
        ...
        if (config != null && config.getConfigurationType() != null) {
            ...
            if ("DEFAULT".equalsIgnoreCase(type)) { //$NON-NLS-1$
                ...
                type = ExtendedJavaTypeResolverDefaultImpl.class.getName();
            }
        } else {
            type = JavaTypeResolverDefaultImpl.class.getName();
        }
    ...
    public static CommentGenerator createCommentGenerator(Context context) {
        ...
        String type;
        if (config == null
                || config.getConfigurationType() == null
                || "DEFAULT".equalsIgnoreCase(config.getConfigurationType())) {
            type = DefaultCommentGenerator.class.getName();
        } else if ("EXTENDED".equalsIgnoreCase(config.getConfigurationType())) {
            type = ExtendedDefaultCommentGenerator.class.getName();
        } else {
            type = config.getConfigurationType();
        }
